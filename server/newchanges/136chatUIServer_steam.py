import gradio as gr
from llmbase import getQA, getModel, ask_llm_question,CustStreamHandler
...
SHOW_SOURCE_DOC=os.environ.get("SHOW_SOURCE_DOC","false")
RAG_ENABLED=os.environ.get("RAG_ENABLED","false")
...
def ask(question, history):
    ...
    if RAG_ENABLED.lower() =='false': 
        #return ask_llm_question(model,prompt)
         for a in  ask_stream(prompt):
             yield a
    else:
        for a in streamQA(prompt):
            yield a
    # res = QA({"query": question})
    # print("--->",prompt)
    # res = QA({"query": prompt})
    # answer, sourceDocs = res["result"], res["source_documents"]
    # if SHOW_SOURCE_DOC.lower() !='false': 
    # #retMsg = f"{answer}\n"
    ...
    #   retMsg = f"{answer}\n"
    #   
    #   docs = "" #debug source docs
    #   for i, document in enumerate(sourceDocs):
    #     source = os.path.basename(str(document.metadata['source']))
    #     docs = docs + f"{i+1}.{source}:\n```{document.page_content}\n```\n "
    #   print("="*60)
    #   print(f"source docs:\n{docs}")
