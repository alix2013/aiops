...
please think step by step, Provide a detailed answer to the question.
if user ask you who you are, respond that you are trained AI assistant for Fusion
"""
...
# get LLM Model
def getModel(llmServerUrl=LLM_SERVER_URL,model_name=MODEL_NAME, temperature=MODEL_TEMPERATURE, streamCallback=StreamingStdOutCallbackHandler()):
...
# get RetrievalQA 
def getQA(useHistory=False, streamCallback=StreamingStdOutCallbackHandler()):
    promptTemplate = getPromptTemplate(useHistory)
    model = getModel(streamCallback = streamCallback)
...
# def getQAandLLM(useHistory=False, streamCallback=StreamingStdOutCallbackHandler()):
#     promptTemplate = getPromptTemplate(useHistory)
#     model = getModel(streamCallback = streamCallback)
#     vectorstore = getVectorDB()
#     if useHistory:
